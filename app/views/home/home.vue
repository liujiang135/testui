			...
			<div class="w-cells">
				<div class="row" v-if="index % 4 == 0 && index < 7" v-for="(item, index) in myApps">
					<div class="col" v-for="n in 4">
						<router-link v-if="myApps[index] && index < 7" :to="'/appdetail?appId=' + myApps[index].applicationId + '&beforeRoute=/home'">
							<div class="cell-item">
...
<script>
	...
	var methods = {
		...
		loadmyapp: function() {
			 ...
			 *应用
			 */
			if(this.uId >0){ //如果用户存在，读取收藏应用
                var favoriteApps = JSON.parse(getLocalStorage('favoriteApps'));
                if(favoriteApps) {
                    that.myApps = favoriteApps;
                } else {
                    $loading.show('数据加载中');
                    httpUtils.getRequest('/appstore/favoriteapps', {
                        uId: this.uId
                    }, function(data) {
                        //FIXME 此处的$loading.hide()要与新闻协调。。。
                        $loading.hide();
                ...
                var recommendApps = JSON.parse(getLocalStorage('recommendApps'));
                if(recommendApps) {
                    that.myApps = recommendApps;
                } else {
                    $loading.show('数据加载中');
                    httpUtils.getRequest('/appstore/applist', {uId: this.uId}, function(data){
                        if(data && data.data){
                            $loading.hide();
                            //推荐的应用
                            that.myApps = data.data.recommendApps;
