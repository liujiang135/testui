<template>
	<div id="news">
		<w-header color="blue-bg">
			<router-link :to="callbackurl" class="button button-icon icon ion-ios-arrow-left" slot="left"></router-link>
	      	<div slot="title">
	      			新闻
	      	</div>
  ...
  var methods = {
    ...
    favorite(){
		...
		if(that.uId > 0){
			...
			httpUtils.postRequest(url, {newsId: newsId, userId: that.uId}, function(data){
				var data = data.data;
