  ...
  <transition name="slide">
    <div class="app-search">
      <w-header  color="blue-bg" >
        <router-link to="/appstore" class="button button-icon icon ion-ios-arrow-left header-left-icon" slot="left"></router-link>
        <div slot="title">
      ...
      <div class="w-content w-content-no-bottom">
        <div v-if="keywords">
          ...
          <router-link v-for="(item, index) in apps" :to="'/appdetail?appId=' + item.appId + '&beforeRoute=/appstore'">
            <div class="app-item-deliver"></div>
            <w-list-item class="item-icon-left">
              {{item.appName_cn}}
              <w-icon>
                <img class="app-icon loadappimg" :src="item.appIcon" />
              </w-icon>
            </w-list-item>
          </router-link>
          <!--没有应用-->
        ...
        <div v-else>
          ...
          <router-link v-for="(item, index) in hotApps" :to="'/appdetail?appId=' + item.appId + '&beforeRoute=/appstore'">
            <div class="app-item-deliver"></div>
            <w-list-item class="item-icon-left">
              {{item.appName_cn}}
              <w-icon>
...
<script>
  ...
  import {getLocalStorage} from '../../assets/js/utils/common-utils';
      ...
      if(keys){
        for(var i in keys){
          for(var j in categoryApps[keys[i]]){
            if(categoryApps[keys[i]][j].appName_cn.indexOf(keywords) != -1){
