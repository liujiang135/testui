<template>
  <div von-loading class="loading-container" :class="{'visible': state == 1, 'visible active': state == 2}">
    <div :class="showSpinner == true? 'loading':'loading loading-toast'">
...
<style scoped>
  ...
  .loading-container .loading-toast {border-radius: 20px; padding: 6px 15px;font-size:12px;}
...
<script>
  import {removeElement} from '../../services/utils'
  ...
  export default {
    ...
    destroyed() {
      console.log('[Wingconn] Loading _vm destroyed');
      removeElement('wingconn-loading');
    },
