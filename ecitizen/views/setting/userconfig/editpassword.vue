	...
	<div class="w-body">
		...
		<div class="w-content">
			<div class="editpwd">
				...
				<w-input label="确认密码:" :type="type" v-model="confirmPassword" placeholder="6-12位,数字或字母" ref='confirmPassword'></w-input>
				<p class='tip'>{{confirmPasswordtip}}</p>
...
<script>
	...
	export default {
		...
		mounted: function() {
			...
			var confirmPasswordel = $(that.$refs.confirmPassword.$el).find("input");
			confirmPasswordel.on('blur',function () {
                if(that.newPassword != that.confirmPassword) {
                    that.confirmPasswordtip = "两次输入的密码不一致";
                }else{
                    that.confirmPasswordtip = "";
                }
            });
		},
		methods: {
			submit: function() {
				...
				$loading.show('数据处理中...');
				httpUtils.postRequest("/userinfo/editpassword", {
                    newPassword: that.oldPassword.toString(),
                    confirmPassword: that.confirmPassword.toString(),
                    oldPassword: that.oldPassword.toString()
				}, function(data) {
                    var data = data.data;
                    if(data.statusCode == 200) {
                        if(data.code == 200 && data.result == true){
                            ...
                            $toast.show(data.response, 1000);
						}
                    }else{
                        $toast.show(data.error, 1000)
					}
				})
			},
			toggle: function() {
